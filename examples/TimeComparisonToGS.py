...
for fact_row in sisu_table.rows:
    ...
    ws.update_cell(idx, 9, fact_row.previous_period_size)
    ws.update_cell(idx, 10, fact_row.recent_period_size)
    ws.update_cell(idx, 11, fact_row.previous_period_value)
    ws.update_cell(idx, 12, fact_row.recent_period_value)
    ws.update_cell(idx, 13, str(fact_row.previous_period_start))
    ws.update_cell(idx, 14, str(fact_row.previous_period_end))
    ws.update_cell(idx, 15, str(fact_row.recent_period_start))
    ws.update_cell(idx, 16, str(fact_row.recent_period_end))
