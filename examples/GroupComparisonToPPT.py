...
for fact_row in sisu_table.rows:
    ...
    s.shapes[1].text = s.shapes[1].text + ', Sisu found that the average ' + METRIC_NAME + ' ' + factor_change_dir + ' ' + str(abs(factor_change_pct)) + '% (from ' + str(round(fact_row.group_a_value, 1)) + ' to ' + str(round(fact_row.group_b_value, 1)) + '.) This ' + factor_change_dir + ' the overall average ' + METRIC_NAME + ' between groups by ' + '<impact score change>.'
